...
def test_handle_incoming_transaction():
    ...
    success, wallet = RoomManagementAPI.create_new_wallet(user1.matrix_id, [user2.email], "test_wallet", 2,2)
    # wallet_id = '!yhwaOvIUxexkOTm
    # LPt:matrix.org'
    assert success
    # user1 generates a transaction
    # when user1 sends a transaction to the wallet
    TransactionsAPI.generate_transaction_and_send_to_wallet(user1.matrix_id, wallet.wallet_id, "test_transaction", "test_transaction")
